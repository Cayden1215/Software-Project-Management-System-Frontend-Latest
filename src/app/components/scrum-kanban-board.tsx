...
export function ScrumKanbanBoard({ project, currentUser, isManager, onUpdateProject }: ScrumKanbanBoardProps) {
  ...
  const refreshTasks = async () => {
    ...
    const updatedSprints = project.sprints.map((sprint) => ({
      ...sprint,
      taskIds: tasks.filter((t) => t.sprintId === sprint.id).map((t) => t.id),
    }));

    onUpdateProject({ ...project, tasks, sprints: updatedSprints });
  };

  useEffect(() => {
    setSelectedSprint((current) => {
      if (project.sprints.length === 0) return null;
      if (current && project.sprints.some((s) => s.id === current.id)) return current;
      const activeSprint = project.sprints.find((s) => s.status === 'active');
      return activeSprint || project.sprints[0];
    });
  }, [project.sprints]);
  ...
  const handleAddToSprint = async (project: Project, taskId: string, sprintId: string) => {

    try {
      setIsLoading(true);
      const sprintID = parseInt(sprintId);
      const taskID = parseInt(taskId);
      const projectID = parseInt(project.id);
      ...
      if (task != null) {
        ...
        await taskAPI.updateTask(parseInt(project.id), taskID, taskDto);
      }
      ...
      alert('Failed to add task to sprint. Please try again.');
    } 
    finally {
      setIsLoading(false);
    };
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                {selectedSprint && view === 'sprint' && (
                  <div className="flex items-center gap-2">
                    <span className={`px-3 py-1 rounded-full text-sm ${selectedSprint.status === 'active' ? 'bg-green-100 text-green-700' :
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors ${view === 'sprint'
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors ${view === 'backlog'
                    ? 'bg-white text-gray-900 shadow-sm'
                    : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {project.sprints.map(sprint => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg whitespace-nowrap transition-colors ${selectedSprint?.id === sprint.id
  ...
  function TaskCard({ task, allTasks, project, currentUser, isManager, onDragStart, onClick, onRemoveFromSprint }: TaskCardProps) {
    ...
    return (
      <div
        ...
        className={`bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all border-2 group ${isMyTask
  ...
  interface BacklogListProps {
    ...
    onAddToSprint: (project: Project, taskId: string, sprintId: string) => void;
  }
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 z-10 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`px-4 py-2 border rounded-lg flex items-center gap-2 transition-colors ${showFilters || activeFiltersCount > 0
                          ...
                          <div className="absolute right-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                            {project.sprints.map(sprint => (
                              <button
                                key={sprint.id}
                                onClick={() => {
                                  onAddToSprint(project, task.id, sprint.id);
